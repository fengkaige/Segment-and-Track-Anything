...
def res_by_num(input_video, input_img_seq, frame_num):
    ...
    output_mask_path = sorted(
        [
            os.path.join(output_mask_dir, img_name)
            for img_name in os.listdir(output_mask_dir)
        ]
    )
...
def choose_obj_to_refine(
    input_video, input_img_seq, Seg_Tracker, frame_num, evt: gr.SelectData
):
    ...
    with app:
        gr.Markdown(
            ...
            <div style="text-align:center;">
                <span style="font-size:3em; font-weight:bold;">My IVOS Matting Visualization</span>
            </div>
            """
        )
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        with gr.Accordion("roll back options", open=False):
                            ...
                            # with tab_show_res:
                            output_res = gr.Image(
                                label="Segment result of all frames"
                            ).style(height=550)
                            frame_per = gr.Slider(
                            ...
                            frame_per.release(
                                show_res_by_slider,
                                inputs=[input_video, input_img_seq, frame_per],
                                outputs=[output_res, frame_num],
                            )
                            roll_back_button = gr.Button(
                                value="Choose this mask to refine"
                            )
                            refine_res = gr.Image(label="Refine masks").style(
                                height=550
                            )
        ...
        output_res.select(
            ...
            inputs=[input_video, input_img_seq, Seg_Tracker, frame_num],
        ...
        roll_back_click_undo_but.click(
            ...
            outputs=[Seg_Tracker, refine_res, click_stack],
        )
        ...
        roll_back_track_for_video.click(
            fn=tracking_objects,
            inputs=[Seg_Tracker, input_video, input_img_seq, fps, frame_num],
            outputs=[output_video, output_mask],
        )
