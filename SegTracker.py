...
class SegTracker:
    ...
    def seg(self, frame):
        ...
        for id in obj_ids:
            if (
                np.sum(self.origin_merged_mask == id) < self.min_area
                or self.object_idx > self.max_obj_num
            ):
    ...
    def detect_and_seg(
        ...
        for i in range(len(boxes)):
            ...
            if (bbox[1][0] - bbox[0][0]) * (
                bbox[1][1] - bbox[0][1]
            ) > annotated_frame.shape[0] * annotated_frame.shape[
                1
            ] * box_size_threshold:
                continue
            interactive_mask = self.sam.segment_with_box(
                origin_frame, bbox, reset_image
            )[0]
            refined_merged_mask = self.add_mask(interactive_mask)
    ...
    def detect_and_seg_vitmatte(
        ...
        for i in range(len(boxes)):
            ...
            # pdb.set_trace()
            """
            origin_frame shape : (720, 1280, 3)
            bbox : array(  [[ 719,   73],
                    [1098,  717]])
                L, U; R, D
            origin_frame[bbox[0][1]:bbox[1][1], bbox[0][0]:bbox[1][0]] shape : (717, 1098, 3)
            bbox_img = origin_frame[bbox[0][1]:bbox[1][1], bbox[0][0]:bbox[1][0]] shape : (644, 379, 3)
            """
            interactive_mask = self.sam.segment_with_box(
                origin_frame, bbox, reset_image
            )[0]
            ...
            ###### >>>>>>
            print("interactive_mask shape before bbox :", interactive_mask.shape)
            bbox_interactive_mask = interactive_mask[
                bbox[0][1] : bbox[1][1], bbox[0][0] : bbox[1][0]
            ].copy()

            bbox_img = origin_frame[
                bbox[0][1] : bbox[1][1], bbox[0][0] : bbox[1][0]
            ].copy()

            vitmatte_mask = run_vit_matte_func(
                bbox_img,
                bbox_interactive_mask.astype(np.uint8),
                # bbox_img,
                # np.array(bbox_interactive_mask == True).astype(np.uint8),
            )
            vitmatte_mask[vitmatte_mask > 0.5] = 1
            vitmatte_mask[vitmatte_mask < 0.5] = 0

            # interactive_mask = run_vit_matte_func(origin_frame, interactive_mask)
            # interactive_mask = np.pad(
            #     vitmatte_mask,
            #     (
            #         (bbox[0][1], origin_frame.shape[0] - bbox[1][1]),
            #         (bbox[0][0], origin_frame.shape[1] - bbox[1][0]),
            #     ),
            #     "constant",
            #     constant_values=(0, 0),
            # )
            interactive_mask[bbox[0][1] : bbox[1][1], bbox[0][0] : bbox[1][0]] = (
                vitmatte_mask
            )
            print("interactive_mask shape after padding :", interactive_mask.shape)
            ###### <<<<<<
...
if __name__ == "__main__":
    ...
    origin_frame = cv2.imread(
        "/data2/cym/Seg_Tra_any/Segment-and-Track-Anything/debug/point.png"
    )
    ...
    predicted_mask, annotated_frame = Seg_Tracker.detect_and_seg(
        origin_frame, grounding_caption, box_threshold, text_threshold
    )
